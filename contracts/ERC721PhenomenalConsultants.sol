 ...
 */
contract ERC721PhenomenalConsultants is ERC721, ERC721Enumerable, Ownable {
    using SafeERC20 for IERC20;
    IERC20 public immutable usdt;
    GWT public immutable gwt;
    ...
    mapping(uint256 => uint256) private RarityLevels;// token ID -> rarity level
    mapping(uint256 => uint) private NftLastClaimDate;//last date when NFT owner claimed GWT profit
    ...
    {   
        require(address(_usdtTokenAddress)!=address(0),'[CNS] Non zero address');
        require(_feeCollector!=address(0),'[CNS] Non zero address');
        require(address(_gwt)!=address(0),'[CNS] Non zero address');
        require(_numberOfTokens!=0,'[CNS] Supply is 0');
    ...
    function setBatchRarityLevels(uint16[] calldata tokenIds, uint16[] calldata rarityLevels) external onlyOwner {
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            ...
            tokenId = tokenIds[i];
            require(RarityLevels[tokenId] == 0, "Rarity already set");
            require(rarityLevels[i] < upperRarityBound,'Rarity < upperRarityBound');
            RarityLevels[tokenId] = rarityLevels[i];
    ...
    function mintNFT(address receiverAddress) external onlyAllowed {
        ...
        require(RarityLevels[freeTokenIdx]>0,'[CNS] No rarity for token');//token should have rarity number
        //get tokenID
    ...
    function _mint(address receiverAddress, uint tokenId, uint freeTokenIdx) private {
        ...
        NftLastClaimDate[tokenId] = block.timestamp;
        //increase receiver total rarity
        AddressRarity[receiverAddress] += RarityLevels[tokenId];

        releasedRarity += RarityLevels[tokenId];
    ...
    function batchClaimGwtProfit() public {
        ...
        for (uint i=0; i < tokenIds.length; i++) {
            ...
            NftLastClaimDate[tokenIds[i]] = block.timestamp;
            //emit event
    ...
    function _calcNftGwtProfit(uint256 tokenId) view public returns(uint){
        ...
        uint secondsPassed = block.timestamp - NftLastClaimDate[tokenId];//seconds since last claim
        uint claimableGwt = (upperRarityBound -  RarityLevels[tokenId]) * profitPerSecond * secondsPassed;//lowest quality minus current quality * time passed
    ...
    function transferFrom(
        ...
        AddressRarity[from] -= RarityLevels[tokenId];
        AddressRarity[to] += RarityLevels[tokenId];
    ...
    function changeFeeCollector(address newCollector) external onlyOwner {
        require(newCollector!=address(0),'[CNS] Non zero address');
