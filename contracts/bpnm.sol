 ...
 */
contract bpnmMain is IERC20Metadata, Ownable {
    ...
    PLD public _pld;
    GWT public gwt;
    Oracles public _chainLinkBtcOracle = Oracles(0x264990fbd0A4796A3E3d8E37C4d5F87a3aCa5Ebf);
    Oracles public _binanceBtcOracle = Oracles(0x83968bCa5874D11e02fD80444cDDB431a1DbEc0f);
    ...
    */
    ...
    function _buyLimitPack(uint packID, address buyerAddress, uint paymentID) private returns (bool){
        ...
        users[buyerAddress].buyLimitLeft += limitPacks[packID].cost*buyLimitMultiplier*1e18/getBtcPrice();
        //increase amount of buy limit deposited by limit packs for all time
        userOverLimits[buyerAddress].totalBuyLimit += limitPacks[packID].cost*buyLimitMultiplier*1e18/getBtcPrice();

        //accrue gwt compensation
        if (prestartMode) {
            //x2 for prestart
            require(gwt.mint(buyerAddress,limitPacks[packID].cost/100*limitPackPurchaseGwtCompensation*2),'[bPNM] GWT mint error');
        } else {
            require(gwt.mint(buyerAddress,limitPacks[packID].cost/100*limitPackPurchaseGwtCompensation),'[bPNM] GWT mint error');
        ...
        _paymentToken.safeTransfer(feeCollector, limitPacks[packID].cost/100*feeCollectorFromPack);


        //transfer to liquidity not distributed rewards, substract discount granted by NFT
        _paymentToken.safeTransfer(liquidityCollector, moveToLiquidity+(limitPacks[packID].cost/100*liquidityCollectorFromPack)-discountAmount);
    ...
    function _uplineBonusDistribution(address buyerAddress, uint processedAmount, bool isMatchingBonus, uint paymentID) private returns(uint leftover){
        ...
        while(true) {
            ...
            if (!_isLvlOpened(uplineUsers[activeIndex],activeLevel)&&isMatchingBonus) {
                ...
                accruedBonus = _depositFrozenBonusToUser(matchingBonusAmount/100*treeLvls[activeLevel].frozenPercent,uplineUsers[activeIndex],activeLevel,paymentID);
            ...
            if (!_isLvlOpened(uplineUsers[activeIndex],activeLevel)&&!isMatchingBonus) {
                ...
                accruedBonus = _depositFrozenBonusToUser(packCost/100*treeLvls[activeLevel].bonus/100*treeLvls[activeLevel].frozenPercent, uplineUsers[activeIndex], activeLevel, paymentID);
                usedBonus += accruedBonus;
            ...
            if (_isLvlOpened(uplineUsers[activeIndex],activeLevel)&&!isMatchingBonus) {
                ...
                accruedBonus = _depositBonusToUser(packCost/100*bonusPercent, uplineUsers[activeIndex], paymentID);
                usedBonus += accruedBonus;
        ...
        if (isMatchingBonus) {
            ...
            return(processedAmount/100*marketingDistributionFromPack-usedBonus);
        }
    ...
    function _depositBonusToUser(uint depositAmount, address receiver, uint paymentID) private returns (uint depositedAmount){
        ...
        if (users[receiver].limitPackId >= 5) {
            depositAmount = depositAmount/100*90;
        }
    ...
    function buyBpnm(uint btcbAmount) public onlyActivated onlyVerified notPrestart onlyUnlocked{
        ...
        uint collectorFee = btcbAmount/100*bpnmBuyFee;
    ...
    function sellBpnm(uint bpnmAmount) public onlyActivated notPrestart{
        ...
        uint collectorFee = bpnmAmount*sellBpnmPrice/100*bpnmSellFee/1e18;
    ...
    function withdrawBalance(uint withdrawAmount, uint compensateGWT, uint paymentID) public {
        ...
        uint matchingPayment = withdrawAmount/100*withdrawMinFee;
        //max fee that can be applied excluding matching bonus
        uint maxFee = withdrawAmount/100*withdrawBaseFee - matchingPayment;
        ...
        if (totalNftRarity > 0) {
            ...
            nftDiscountAmount = withdrawAmount / 1e6 * (1200*nftOwned-totalNftRarity) * nftDiscountForWithdraw;
    ...
    function extendMatchingBonus() public onlyActivated{
        ...
        if (totalNftRarity > 0) {
            uint nftOwned = nftCollection.balanceOf(msg.sender);
            discountAmount = matchingBonusGwtCost / 1e6 * (nftOwned*1200 - totalNftRarity) * nftDiscountForMatchingPayment;
            if (discountAmount > matchingBonusGwtCost/100*30) {
                //discount can not be more than 30% of cost
                discountAmount = matchingBonusGwtCost/100*30;
            }
    ...
    function buyEarnLimitWithGwt(uint earnLimitAmount) public onlyActivated{
        ...
        require(userOverLimits[msg.sender].totalEarnLimit/100*earnLimitExtra>=earnLimitAmount+userOverLimits[msg.sender].purchasedEarnLimit, "[bPNM] Amount exceeds 10%");
    ...
    function buyPurchaseLimit(uint usdtPurchaseLimitAmount) public onlyActivated {
        ...
        require(userOverLimits[msg.sender].totalBuyLimit/100*buyLimitExtra>=btcbPurchaseAmount+userOverLimits[msg.sender].purchasedBuyLimit, "[bPNM] Amount exceeds 10%");
    ...
    function buySellLimit(uint usdtSellLimitAmount) public onlyActivated {
        ...
        require(userOverLimits[msg.sender].totalSellLimit/100*sellLimitExtra>=btcbSellAmount+userOverLimits[msg.sender].purchasedSellLimit, "[bPNM] Amount exceeds 10%");
    ...
    function extendLvlMarketingBonus() public onlyActivated {
        ...
        if (totalNftRarity > 0) {
            uint nftOwned = nftCollection.balanceOf(msg.sender);
            discountAmount = nextLvlCost / 1e6 * (1200*nftOwned - totalNftRarity) * nftDiscountForAdditionalMarketingPercent;
    ...
    function purchaseMarketplaceItem(uint itemID) public {
        ...
        if (marketplace[itemID].isLiquidityCompensated) {
            uint collectorFee = marketplace[itemID].bpnmPrice*sellBpnmPrice/100*bpnmSellFee/1e18;
    ...
    function setBtcOracle(uint _selector) external onlyOwner {
        require((_selector >=0)&&(_selector <=2),"[bPNM] Incorrect oracle selector");
    ...
    function setBpnmBuyFee(uint amount) external onlyPromoter {
        require(0<=amount&&amount<=20, '[bPNM] Out of range');
        bpnmBuyFee = amount;
    }

    function setBpnmSellFee(uint amount) external onlyPromoter {
        require(0<=amount&&amount<=10, '[bPNM] Out of range');
        bpnmSellFee = amount;
    }
    ...
    function setgwtTransFeeLiquidity(uint amount) external onlyPromoter {
        require(0<=amount&&amount<=2e18, '[bPNM] Out of range');
